...
#   hw5_utilities.py
#
#   Two classes to help the localization problem set:
#
...
#   The robot.Position() is intended for debugging and visualizatio
...
#   prob          NumPy 2D array of probabilities, i.e. values 0 to 1.
#
#   probCmd       Probability the command is executed (0 to 1).
#   probProximal  List of probabilities (0 to 1).  Each element is the
#                 probability that the proximity sensor will fire at a
#                 distance of (index+1 = 1,2,3,etc)
#
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                if self.walls[row,col]:
                    ...
                    pmin   = 0.9 / self.spots
                    rlevel = (1.0 - p)
                    glevel = (1.0 - p) / max((1, p/pmin))
                    blevel = 1 if p > 0 else 0
